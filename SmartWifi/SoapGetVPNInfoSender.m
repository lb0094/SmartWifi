...
@implementation SoapGetVPNInfoSender
...
//    NSLog(@"elementName= %@",elementName);
    ...
    if ([elementName isEqualToString:matchingElement3]) {
        if (!soapResults3) {             soapResults3 = [[NSMutableString alloc] init];
        }
        elementFound3 = YES;
    }
...
//    if (![soapResults5 isEqualToString: nil]) {
        if ([elementName isEqualToString:matchingElement6]) {
//            NSLog(@"lb0094 elementName= %@",elementName);

            if (soapResults5 != nil) {
                soapResults6 = [[NSMutableString alloc] init];
//                NSLog(@"lb2 elementName= %@",elementName);
            }
            elementFound6 = YES;
        }
//        if (!soapResults6) {
...
//    if (![soapResults5 isEqualToString: nil] || ![soapResults5 isEqualToString:@""]) {
        if (elementFound6) {
            [soapResults6 appendString: string];
//            NSLog(@"lb= %@",soapResults6);
        }
//    }
...
//        [xmlParser abortParsing];
    ...
    if ([elementName isEqualToString:matchingElement7]) {
//        NSLog(@"lbtest= %@",soapResults2);
        if ([soapResults2 isEqualToString:@"eHour"]) {      //包小时
//            NSLog(@"lbtest type= %@ totalHours= %@",soapResults2,soapResults4);
            [[NSNotificationCenter defaultCenter] postNotificationName:@"getVpnInfo" object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:soapResults2, @"type", soapResults4, @"totalHours" , nil]];
        }else if([soapResults2 isEqualToString:@"eFlow"]){      //包流量

            [[NSNotificationCenter defaultCenter] postNotificationName:@"getVpnInfo" object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:soapResults2, @"type", soapResults3, @"totalFlow" , nil]];
        }else if([soapResults2 isEqualToString:@"eMonth"]){         //包月
            

            [[NSNotificationCenter defaultCenter] postNotificationName:@"getVpnInfo" object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:soapResults2, @"type", soapResults5, @"year" , soapResults6, @"month" , soapResults7, @"day" , nil]];
        }
